...
<div id="content">
    ...
    <?php
        ...
        $e_d_id=$_GET['id'];
        $query = "SELECT * FROM `emp_dep` WHERE `id_emp_dep`='$e_d_id'";
        mysqli_query($db,  $query) or die(mysqli_error($db)); 
        $emp_dep=mysqli_query($db, $query);
        $emp_dep=mysqli_fetch_assoc($emp_dep);

        if (!empty($_POST)) {
            $id=$_POST['id'];
            $num_emp = $_POST['num_emp'];
            $num_dep = $_POST['num_dep'];
            $query = "UPDATE `emp_dep` SET `num_emp` = '$num_emp', `num_dep` = '$num_dep' WHERE `emp_dep`.`id_emp_dep` = '$id'"; 
            mysqli_query($db,  $query) or die(mysqli_error($db)); 
            header('Location: emp_dep.php');
        }

    ?>
    <form method="post" action="edit_e_d.php">
            <input type="hidden" name="id" value="<?=$emp_dep['id_empl']?>">
            <label>ID сотрудника <input type="text" name="num_emp" value="<?=$emp_dep['num_emp']?>" required></label>
            <label>ID отдела<input type="text" name="num_dep"  value="<?=$emp_dep['num_dep']?>" required></label><br>
